...
class IdentityAllocationTests(unittest.TestCase):
    ...
    def test_project_configuration_prepares_the_next_patch_successor(self) -> None:
        self.assertEqual(BRIDGE.marketing_version(), "0.2.7")
    ...
    def test_identity_allocation_uses_only_the_registered_app_and_highest_build(self) -> None:
        ...
        self.assertEqual(proof["marketingVersion"], BRIDGE.marketing_version())
    ...
    def test_identity_allocation_advances_past_a_locally_failed_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            version = BRIDGE.marketing_version()
            manifest = root / f".git/release-state/wilted-ios/candidates/{version}-1/manifest.json"
            manifest.parent.mkdir(parents=True)
            manifest.write_text(json.dumps({"immutable": True, "release": {"marketingVersion": version, "buildNumber": "1"}}), encoding="utf-8")
