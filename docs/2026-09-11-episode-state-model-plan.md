...
Review basis: `2d7747282eb2f8d8523399346d9ba9a53d52a1fb`, drafted while the working tree carried uncommitted changes to `LocalLibraryStore.swift`, `PodcastDownloadCoordinator.swift`, `PodcastPreparationPipeline.swift`, `WiltedMacModel.swift`, and their tests. Those changes landed in `2d77472` itself, and this document has since been independently re-verified line-by-line against that HEAD, twice (`3850029`, then `4a5aa74`). Every line reference below is exact as of `4a5aa74`; re-verify before editing regardless, since implementation work will move these lines. `docs/2026-09-11-lifecycle-reliability-deep-plan.md`, referenced below, is a local working document excluded from this public repository by `.gitignore`; readers without local access should treat its "ten-value lifecycle contract" claim as reported, not linkable.
...
The injection seams for real-wiring tests are half present. `PodcastDownloadCoordinator.init` already takes `transport: any PodcastDownloadTransporting = URLSessionPodcastDownloadTransport()` and `mediaValidator: any PodcastMediaValidating = AVFoundationPodcastMediaValidator()` (coordinator 154-168, defaults at 157-158) and `PodcastPreparationPipeline.init` already takes `runner: any PodcastPipelineRunning = SubprocessPodcastPipelineRunner()` (pipeline 552-570, default at 555). The gap is that `WiltedMacModel.configureStoreDependencies(_:)` (4473-4528, construction at 4499-4508) constructs both with defaults and `WiltedMacModel.init(...)` (1208-1219) exposes no factory for either. Separately, `downloadEpisode` has an entire parallel fixture implementation (2007-2040) that fakes progress and failure without touching the coordinator, so UI-fixture coverage proves nothing about the real path. `AVFoundationPodcastMediaValidator` requires genuinely playable audio bytes, so a fixture transport supplying placeholder bytes needs the validator seam too, not just the transport — this was missed in the original survey and is corrected in Phase 1's description below.
...
**3. Which state may be ephemeral.** Exactly three things stay in memory: `podcastDownloadTasks` and the running-preparation set (this process's in-flight work, legitimately unknown to the store); `hiddenEpisodeIDs` (optimistic hide, reconciled by the store result on the next snapshot); and `podcastOperationMessage` (transient user-facing text). The `.prepared(summary:)` override at 2374 is removed. `applyingRunningPreparations(to:from:running:)` (2908-2923) is retained but narrowed to carrying `.preparing` only — never a terminal state. `DeferredAutomaticPreparation` in `UserDefaults` (2133-2177, restored at 2258-2273) is durable state in the wrong store and moves to the library in Phase 8.
...
| `downloading` | `invalidURL`, `insecureRedirect`, `unsupportedMediaType`, `mediaTypeMismatch`, `declaredSizeTooLarge`, `streamedSizeTooLarge`, `declaredSizeMismatch`, `invalidExpectedHash`, `hashMismatch`, `invalidAudio`, `destinationExists` | `failed(terminal)` | status + `failureKind` | throws; automation does not retry, counts as not downloaded |
...
`episodeNotFound` is not in this table: it is thrown from a guard before the coordinator saves any download row (`PodcastDownloadCoordinator.swift:183-185`), so there is no row to transition and nothing for `failureKind` to classify. It means the caller (automation or UI) asked to download an episode absent from the local library — a caller-side precondition failure, not a transfer outcome. Phase 3 must decide how the automation adapter surfaces this distinctly from a real transfer failure, since counting it as any kind of "failed download" would blame the download layer for a library-state mismatch.
...
| `hiddenEpisodeIDs` | model 1191 | legitimate optimistic UI, reconciled by the store result |
...
Add `podcastDownloadTransportFactory`, `podcastMediaValidatorFactory`, and `podcastPipelineRunnerFactory` parameters to `WiltedMacModel.init(...)` (1208-1219) and thread them through `configureStoreDependencies(_:)` (4473-4528). Collapse the fixture download branch (2007-2040) onto the real path behind a fixture `PodcastDownloadTransporting` and `PodcastMediaValidating` pair (both needed: the default `AVFoundationPodcastMediaValidator` rejects placeholder audio bytes) that emit the same progress and failure shapes, so `--wilted-ui-fixture-download-failure` exercises production code. No behavior change, no schema change.

Landed as two commits: 1a (the three DI seams, defaulting to `nil`, zero behavior change) and 1b (the fixture branch collapse, using fixture-mode default factories when no explicit factory is injected).
...
Change the task table type, make `startDownload` throwing, make `startClaimedDownload` propagate, add `failureKind` classification in the coordinator's final catch (375-386), add `resumablePodcastDownloads()`, and wire bootstrap reconciliation (`performStoreBootstrap()`, 4358-4431) to it. The UI wrapper presents failures; orchestration receives them. The final catch cannot classify every case in the enum: `episodeNotFound` and the early `CancellationError`→`.cancelled` remap (~191-198) both throw before any `queued` row exists, so they never reach this catch and this phase's classification logic never sees them. The caller-side surfacing of `episodeNotFound` is a separate decision (see owner decision 1).
...
Move the outcome insert into `replaceReadyRevision`'s save, move source deletion out of `commit(...)` into a separate reclamation step, delete the `.prepared(summary:)` override (2374), repoint `preparationState(...)` at the outcome row, and gate `closeInterruptedPreparationRuns`. `applyingRunningPreparations` already guards `.preparing` only (`WiltedMacModel.swift:2913`); this phase does not touch it, and the earlier "narrow to `.preparing`" framing in the facts-and-boundaries section above is the current state, not new work.
...
2. **"Coordinator comments say failed work can be resumed later."** Partially right, but not about the coordinator, and not about `.failed` in general. Three comments in `WiltedAutomationCoordinator.swift` (57, 207, 309-310) do say a failed or claimed-but-unstarted episode is picked up on a later run. Today that promise is true only for `claimedEpisodeMissing` and the resumable set defined by `unfinishedPodcastDownloads()` (`LocalLibraryStore.swift:2585-2593`: "`queued` is claimed and not started, `downloading` is a transfer with no process behind it any more. Both are resumable, and the store is the only thing that knows which episodes they are.") — it does not cover a real transfer failure, because `.failed` is excluded from that set today. The comments describe the intended behavior for the whole class of interrupted work, not the current behavior for `.failed`; Phase 3's `resumablePodcastDownloads()` is what makes the comment's promise hold for `.failed(retryable)` too, rather than only for the claim-without-start case. The issue's framing ("comments say X, code does not do X") is directionally correct for failures; it is wrong only in implying no such comment exists at all.
...
Everything else in the issue matched the code, including the `preparationRuns(limit: Int.max)` call (4719), the `.prepared(summary:)` override (2374), the successor/no-successor asymmetry, the absence of a file-existence check in `readyRevision` (1597-1607), the fingerprint's inclusion of speech-stack Python sources (476-536), and the restore-returns-bare-row behavior (2617-2645).

## Decisions that need the repo owner

1. **Automation download retryability.** Default adopted above: `transport` and `invalidResponse` are `retryable` — bounded in-process retry via the existing `withRetries`, claim stays `queued`, re-queued at next launch; `invalidURL`, `insecureRedirect`, `unsupportedMediaType`, `mediaTypeMismatch`, `declaredSizeTooLarge`, `streamedSizeTooLarge`, `declaredSizeMismatch`, `invalidExpectedHash`, `hashMismatch`, `invalidAudio`, `destinationExists` are `terminal` — no auto-retry, excluded from relaunch reconciliation, user action only; `cancelled` is neither. `episodeNotFound` and the early `CancellationError` remap never reach this classification at all (they precede the `queued` row save), so they need no retryability decision — only a decision about how the automation adapter surfaces a precondition failure distinctly from a transfer failure. The genuinely open half is whether a `retryable` failure should be re-queued automatically at next launch (default here: yes) or wait for the user, since automatic re-queue means a persistently dead feed retries on every launch forever.
