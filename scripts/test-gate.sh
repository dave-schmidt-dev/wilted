...
assert_mac_ui_selector_floor_contract() {
  ...
  [[ "$(unset WILTED_MAC_UI_SELECTOR; expected_test_count_floor macos-ui-tests)" == "14" ]] ||
    fail 'default Mac UI suite must retain its fourteen-test floor'
...
assert_result_bundle_tests() {
  ...
  if [[ "$native_self_test" == "1" ]]; then
    if is_forced_zero "$label"; then
      ...
      printf '%s\n' '{"totalTestCount":14}' >"$summary_file"
