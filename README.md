...
Navigation is a destination switch on both platforms. The Mac window is a two-column split view whose sidebar lists Library, Now Playing, and Settings; exactly one destination fills the detail region, and playback survives switching between them. iPhone always exposes Library, Now Playing, Downloads, and Settings as tabs. Downloads is listener-only: Mac audio is local the moment it is produced. The candidate-current route inventory is [`docs/2026-08-25-mac-ios-walkthrough.md`](docs/2026-08-25-mac-ios-walkthrough.md).
