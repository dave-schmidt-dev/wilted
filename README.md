...
Larder takes one address box that works out for itself whether a pasted HTTPS address is an article or a podcast feed, so nothing asks the reader to classify a link before pasting it; feed upkeep -- the subscription list, the per-feed switch, unsubscribe, and Refresh -- lives on its own Podcast feeds destination. Navigation is a destination switch on both platforms. The Mac window is a two-column split view whose sidebar lists Larder, Podcast feeds, Prep, and Settings; Now Playing is an always-visible bottom rail, with Transcript and Up Next inline in that rail, and playback survives switching destinations. iPhone exposes Larder, Now Playing, and Settings as tabs; download state and controls live in Larder rather than a duplicate Downloads tab. A dated Mac route inventory as of the 2026-09-01 capture is the self-contained [`docs/2026-09-01-mac-daily-driver-walkthrough.html`](docs/2026-09-01-mac-daily-driver-walkthrough.html). It is a snapshot of the commit it names, not a claim about HEAD: a walkthrough is refreshed for a deployment trigger, not for every UI change, because a locally installed build is reviewed by clicking through it. It does not claim owner acceptance.

Removing an episode from the Larder is permanent and survives a refresh. It has to be recorded rather than merely deleted: the feed still lists the episode, so the next refresh parses it out of the same XML and would insert it again. Store version 8 adds an episode-dismissal record, `savePodcastEpisodes` refuses any identity that carries one, and the episode row, its queue entry, download record, saved speed, and artwork are deleted outright. Downloaded audio files stay on disk for the reason unsubscribing leaves them: a `RevisionID` is derived from content, so two episodes with identical bytes share one revision. Unsubscribing forgets a feed's dismissals along with the rest of its records, so resubscribing starts from the feed rather than from an invisible blocklist.

Preparing a downloaded episode is automatic and runs in one worker process, `Producer/Workers/wilted_pipeline.py`: transcript first (the feed's published transcript, else local speech-to-text aligned to the audio, else the episode page's prose), then ad detection and an ffmpeg cut, then the cues are remapped onto the cut audio so the transcript follows playback. The worker is Python because the ad detector it wraps is the previous project's tuned `wilted.ads`, which is imported from `wilted-old/src` under `wilted-old/.venv` rather than reimplemented; `WILTED_PIPELINE_PYTHON`, `WILTED_PIPELINE_WORKER`, `WILTED_PIPELINE_PYTHONPATH`, `WILTED_PIPELINE_TIMEOUT_S`, and `WILTED_PIPELINE_TOOL_PATH` override the locations. It needs the repaired Gemma GGUF under `~/models/gemma-4-repaired` and `ffmpeg`/`ffprobe` reachable from the worker's PATH, which the app extends with `/opt/homebrew/bin` and `/usr/local/bin` because a Finder launch inherits neither. The worker checks both before spending minutes on speech-to-text, loads the model explicitly, and reports a detector whose model never answered as a failed preparation rather than as an episode with no advertisements; its warnings are relayed into the preparation journal. The Larder row says only that an episode is preparing, what a finished run did ("Ready · 5 ads removed (7:22) · transcript synced", leading with the completion state and naming each step's result, read back from the journal after a relaunch), or that it failed and to see Prep. Prep shows what each run is doing in a sentence, with Stop while it runs and Retry when it failed; each run's Show log lists every journalled status in the worker's own vocabulary for anyone working out why a run did what it did. Redoing a good preparation is in the row's … menu.

The rest of the former `wilted-old` directory is reference-only and non-runnable. Its absolute paths, legacy runtime bindings, SQLite data, alias, and retired launchd jobs are intentionally archived. On 2026-08-17, the owner approved transferring the existing GitHub repository to this fresh project; the old Git history was not migrated, and the local archive remains the recovery copy. Retired nightly/hourly scheduling is disabled. The current `wilted` shell alias is stale and must not be treated as an installation.
