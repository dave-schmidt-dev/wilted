...
Navigation is a destination switch on both platforms. The Mac window is a two-column split view whose sidebar lists Larder, Now Playing, Prep, and Settings; exactly one destination fills the detail region, and playback survives switching between them. iPhone exposes Larder, Now Playing, and Settings as tabs; download state and controls live in Larder rather than a duplicate Downloads tab. The candidate-current route inventory is the self-contained [`docs/2026-08-29-mac-ios-screen-walkthrough.html`](docs/2026-08-29-mac-ios-screen-walkthrough.html).
