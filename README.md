...
The attended Development CloudKit round trip completed on 2026-08-18 on a physical iPhone 16 Pro Max, and its evidence is read from the device's own persisted state rather than from an on-screen status: three records (`WiltedItem`, `WiltedRevision`, `WiltedPlaybackState`) each carrying server system fields and a change tag, all three acknowledged by the private zone, zero pending changes, zero conflicts, and an account owner token equal to the Mac producer's. Background playback is proven by a recorded position of 9.25 s across an 8 s hold with the app backgrounded, taken from the live audio clock rather than the display. Playback is served from the local cache; no airplane-mode run was performed, so this is cache-backed rather than a proven radio-off journey.
