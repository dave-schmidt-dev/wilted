...
area: ["src/wilted/engine.py", "src/wilted/tui/**/*.py", "src/wilted/cli.py", "src/wilted/classify.py", "src/wilted/prepare.py", "src/wilted/station_runtime/coordinator.py"]
gate_test: tests/test_coordinator.py
# Gate references: TestModelCoordinator LLM lease serialization, TestRuntimeBootstrap main-thread tqdm-lock ordering, TestRunClassify.test_llm_lifecycle_runs_under_coordinator_lease, and TestPrepareLLMLifecycle.test_llm_phase_runs_under_coordinator_lease.
threshold: 3
...
### INV-6 — Nightly pipeline stages isolate per-item and per-feed failures (one bad item cannot abort the batch), non-speech CLI/management entrypoints stay daemon-independent, speech-producing paths fail loudly at the speech boundary with `probe=True`, no user-facing entrypoint escapes with a raw traceback, and every wrapper actually runs its pipeline/monitor and logs true exit codes
area: ["src/wilted/classify.py", "src/wilted/discover.py", "src/wilted/fetch_cascade.py", "src/wilted/ingest.py", "src/wilted/fetch.py", "src/wilted/prepare.py", "src/wilted/report.py", "src/wilted/cli.py", "src/wilted/transcribe.py", "src/wilted/speech_ready.py", "src/wilted/pipeline_submit.py", "src/wilted/scheduler_tick.py", "scripts/wilted-nightly.sh", "src/wilted/station_runtime/weather_monitor.py", "scripts/wilted-weather-monitor.sh"]
...
# Additional gate reference: tests/test_cli.py::TestMainEntrypoint (non-speech startup skips daemon readiness; TUI probes once before launch and raises loudly when down; `python -m wilted.cli --version` stays daemon-independent), tests/test_cli.py::TestRequireSpeechReady::test_helper_delegates_to_client_with_probe, tests/test_pipeline_handlers.py + tests/test_scheduler_tick.py (speech-capable runner drains preflight once; non-speech cohorts and station-active deferral issue zero probes), and tests/test_transcribe.py::TestTranscribeAudioExceptionContract + TestTranscribeAudioDaemonOnly (tier-3 STT daemon failures, including DaemonUnavailable, surface as typed TranscriptionError subclasses, never a raw AttributeError)
# Additional gate reference: the three general-article fetch paths (discover CHEAP, ingest FULL, fetch.get_text_from_url FULL) now funnel through `fetch_cascade.resolve_article_text`, so a regression there is an INV-6 violation on the nightly-isolation / no-raw-traceback clauses. Gated by tests/test_fetch_static_guards.py (refactor-proof AST/subprocess guards: CHEAP budget never reaches the browser escalation or the `<main>`-scoped retry; no module-top `import trafilatura`; the transport fetch never runs inside `suppress_subprocess_output` — the 2026-04-20 TUI-freeze class) and tests/test_discover.py (T3.3 nightly resilience: per-item isolation so one bad article cannot abort the feed batch, a transport exception resolves to `(None, None)` rather than escaping, and the per-item fetch stays timeout-bounded)
threshold: 3
rationale: `classify.py` has no per-item try/except so one bad LLM response aborts the whole stage (M4); chained/entry paths surface raw tracebacks (M37, M36); and the nightly wrapper must invoke `wilted.cli.main()` through its `__main__` guard, run the pipeline, and log the true exit code (C1). The standalone weather monitor is a second, independent surface for the same C1 bug class: `weather_monitor.py` isolates a bad NWS fetch/parse and malformed individual alert feature, while its wrapper only exits 0 when the module's `__main__` guard ran a real poll and that poll recorded no error (`last_error is None`) — checked per invocation, not via the three-consecutive-failure `health()=="failed"` threshold. M2 made tier-3 STT daemon-only: `transcribe.py` has no isolated-spawn fallback. F5 routed readiness through `speech_ready.require_speech_ready()` (`probe=True`): help, version, clean, and non-speech management commands stay daemon-independent; TUI, play/next/direct playback-export, and speech-capable runner drains (article-cache, compact-briefing, podcast-prepare, normal article-prepare — skip-TTS article prepare is non-speech) probe once at the speech boundary and fail loudly when the daemon is down, leaving queued jobs unchanged.
...
### INV-10 — Only PipelineRunner (via handlers and execution-capability test seams) may construct expensive ML backends outside tests; production modules outside the narrow allowlist must not call ``create_model_coordinator``, ``create_backend``, or tier-3 ``transcribe_audio`` directly
area: ["src/wilted/pipeline_runner.py", "src/wilted/handlers/**/*.py", "src/wilted/execution_capability.py", "src/wilted/classify.py", "src/wilted/prepare.py", "src/wilted/transcribe.py", "src/wilted/llm.py"]
gate_test: tests/test_background_work_invariant.py
# Additional gate reference: tests/test_execution_capability.py (runtime capability denial and dynamic-import bypass)
threshold: 3
rationale: Task 4.2 added runtime execution-capability gating, but direct factory calls from orchestration modules (classify, prepare, transcribe, CLI/TUI) could still bypass the bounded runner and compete with foreground station work. INV-10 pairs AST scanning of production modules with the existing runtime authority so neither guard is the sole line of defense — only ``PipelineRunner`` may issue capability, handlers perform expensive work under that lease, and ``execution_capability.execution_capability_scope`` remains a test-only seam.
