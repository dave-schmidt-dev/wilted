...
- Status: **RESOLVED 2026-08-06 — fixed and confirmed by David in a real terminal** ("clicking works on the morning report"), covered by `test_single_click_selects_a_fresh_report_row`. Verification needed BUG-7 fixed first (no click reached the app before that) and `wilted report --reset` to repopulate the report, since every item had been dismissed.
