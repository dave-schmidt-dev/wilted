...
- Status: **fix committed 2026-08-06, NOT yet confirmed.** `scripts/wilted-runtime.sh` now passes `TERM_PROGRAM` and `LC_TERMINAL` through the `env -i` allowlist. The regression test proves the variables *arrive*; it cannot prove the mouse works, because the failing path only activates when a real iTerm2 answers the mode-2048 probe. This closes only when David clicks something in a real `wilted` session.
