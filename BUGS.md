...
### BUG-7 — Mouse dead app-wide under the launcher: `env -i` stripped the iTerm2 identity vars
...
- Status: **RESOLVED 2026-08-06 — confirmed by David in a real terminal.** `scripts/wilted-runtime.sh` now passes `TERM_PROGRAM` and `LC_TERMINAL` through the `env -i` allowlist, and under the normal `wilted` alias the UI responds to clicks again ("I can click on the other UI elements now"). The regression test could only prove the variables *arrive*; the terminal confirmation is what closed it, because the failing path only activates when a real iTerm2 answers the mode-2048 probe.
- **Root cause: the launcher's `env -i` allowlist stripped `TERM_PROGRAM` and `LC_TERMINAL`, which is how Textual recognises iTerm2.** Full chain:
  1. `wilted` is an alias for `scripts/wilted-runtime.sh`, which re-execs the app under `env -i` with a fixed allowlist. Neither terminal-identity variable was on it.
  2. `textual/_xterm_parser.py:46` derives `IS_ITERM` from exactly those two variables: `os.environ.get("LC_TERMINAL","") == "iTerm2" or os.environ.get("TERM_PROGRAM","") == "iTerm.app"`. Under the launcher both are empty, so `IS_ITERM` is `False`.
  3. The driver always probes in-band window resize at startup (`linux_driver.py:284` → `\x1b[?2048$p`). iTerm2 replies that it supports the mode.
  4. The parser gates that reply on `mode_id == "2048" and constants.SMOOTH_SCROLL and not IS_ITERM` (`_xterm_parser.py:308-312`). `SMOOTH_SCROLL` defaults to on, so with `IS_ITERM` false the guard falls open and an `InBandWindowResize` message is emitted. Textual's own comment on that guard: *"TODO: iTerm is buggy in one or more of the protocols required here"*.
  5. `linux_driver.process_message` handles that message by enabling in-band resize **and calling `_enable_mouse_pixels()`** (`linux_driver.py:467`), which writes `\x1b[?1016h` — SGR-**pixel** mouse mode.
  6. iTerm2 then reports mouse position in **pixels**, and the coordinates land far outside the grid, so a click hits nothing anywhere in the app.
  - Net effect: mouse completely dead, keyboard completely fine, app-wide rather than modal-specific, and invisible to every headless test. `env -i` was defeating a guard Textual added specifically to protect iTerm2 users.
  - **How far step 6 is actually established.** Steps 1–5 are read directly off Textual 8.2.3's source and confirmed at unit level (`IS_ITERM True -> emits: []` vs `IS_ITERM False -> emits: ['InBandWindowResize']`). Step 6 is an inference and the precise sub-mechanism is *not* yet pinned down. Correcting an earlier version of this entry, which claimed `terminal_pixel_size` simply "stays `None`": that is wrong as stated. `_xterm_parser.py:270-272` sets `terminal_size`, `terminal_pixel_size` and `mouse_pixels` **in the same block**, so the rescale guard at lines 84-88 is self-consistent and can never see pixel mode without a scale factor. What is true is narrower: that block only runs when an in-band resize report (`\x1b[48;h;w;ph;pw t`) is actually parsed, and `_enable_mouse_pixels()` fires merely on the terminal *claiming* support (`supported = setting_parameter not in (0, 4)` — so even a `Ps=2` "recognised but off" reply counts). A terminal that advertises mode 2048 but never emits the report therefore gets pixel mouse mode with no scale factor, and pixel values are read as cells. That matches the symptom and matches Textual's deliberately vague guard comment ("iTerm is buggy in one or more of the protocols required here"), but it has not been measured on David's iTerm2. `scratchpad/pixel-mouse-probe.py` reproduces the broken path and settles it; the answer changes only this bullet, not the fix.
- Fix: add `TERM_PROGRAM="${TERM_PROGRAM:-}"` and `LC_TERMINAL="${LC_TERMINAL:-}"` to the `env -i` allowlist. These are terminal identification strings, not secrets, so the least-privilege design is unaffected — the BWS token and unrelated inherited variables are still barred, and the regression test asserts that too. `IS_ITERM` is an `or`, so on this machine `TERM_PROGRAM` alone would be enough; both are passed because the pair is what Textual actually reads, and passing only one leaves the fix dependent on which variable a given terminal happens to set.
- Regression test: `test_runtime_launcher_passes_terminal_identity_through_for_mouse_input` (`tests/test_runtime_launcher.py`), which drives the real script with fake `security`/`bws`/`uv` binaries and asserts both variables reach the inner environment while `BWS_ACCESS_TOKEN`/`UNRELATED_SECRET` still do not. Verified to fail against the unfixed allowlist. Asserted at the launcher, not in a TUI test, because the defect was in the environment handoff — the app was always correct.
- Ruled out along the way:
  - *App-side click handling.* Under headless Pilot (Textual 8.2.3) a single click on `#done-button` reaches its handler, and clicks on the report `DataTable` do move the cursor. Confirmed again by David running `python -m wilted.cli` directly: the mouse worked.
  - *The terminal.* A raw stdin probe using Textual's exact enable order captured `SGR (1006) reports: 200`, `urxvt: 0`, `X10: 0`, `unrecognised bytes: 0`. iTerm2 emits exactly the encoding Textual requests and decodes.
  - *Textual itself.* `python -m textual` (same version, same venv) had a working mouse.
  - *`bws run` and `uv run`.* `uv run`, and the full `env -i` + `uv run` chain, both preserve stdin/stdout as a tty (measured under a PTY). David confirmed the mouse was still dead with `bws run` removed, which is what pointed at `env -i` itself.
  - *An earlier claim of mine that was wrong:* a PTY capture of a **trivial** Textual app with and without the allowlist showed identical mouse enables, and I recorded that as clearing the launcher. It did not — the enables are written identically either way. The divergence is in what Textual does with the terminal's *reply*, which a trivial app that never reaches that code path could not reveal.
...
- **Encoding mismatch RULED OUT 2026-08-06.** Re-running the probe with Textual's exact enable order produced `SGR (1006) reports: 200`, `urxvt (1015): 0`, `X10 (1000): 0`, `unrecognised bytes: 0`. iTerm2 replies in precisely the encoding Textual requests and decodes, so **the entire terminal layer is now cleared** — the emitter, the encoding negotiation, and the wire format are all correct. Retained below because the asymmetry in Textual's parser is real and would be a plausible cause on a terminal that *did* reply in urxvt; it simply is not what is happening here.
- Superseded lead — **encoding mismatch between what iTerm2 replies with and what Textual can parse**:
...
- Status: **fixed** 2026-08-06, covered by `test_single_click_selects_a_fresh_report_row`. Mouse input reached the app again once BUG-7 was fixed, but this specific behaviour is **still unverified by hand**: the report was empty at confirmation time because every item had been dismissed. `wilted report --reset` returns the decided items to the candidate pool and repopulates it for a manual check.
