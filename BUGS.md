...
- Status: **fixed** 2026-08-06 — `scripts/wilted-runtime.sh` now passes `TERM_PROGRAM` and `LC_TERMINAL` through the `env -i` allowlist. Awaiting David's confirmation in the real terminal.
- **Root cause: the launcher's `env -i` allowlist stripped `TERM_PROGRAM` and `LC_TERMINAL`, which is how Textual recognises iTerm2.** Full chain:
  ...
  6. iTerm2 then reports mouse position in pixels. `parse_mouse_code` only rescales when `terminal_pixel_size` is known; otherwise pixel values are consumed as cell coordinates. A click at cell (50,20) arrives as something like (700,300) — far outside the grid, so it hits nothing anywhere in the app.
...
- Fix: add `TERM_PROGRAM="${TERM_PROGRAM:-}"` and `LC_TERMINAL="${LC_TERMINAL:-}"` to the `env -i` allowlist. These are terminal identification strings, not secrets, so the least-privilege design is unaffected — the BWS token and unrelated inherited variables are still barred, and the regression test asserts that too.
