...
- **Current prime suspect: the launcher wraps Wilted in `bws run`.** The `wilted` shell alias points at `scripts/wilted-runtime.sh`, whose last act is `exec "$BWS_BINARY" run -- "$0" --runtime-inner "$@"`, which re-execs `uv run ... python -m wilted.cli` under `env -i`. So the TUI does **not** run as a direct child of the shell — it runs underneath a `bws run` process wrapper. If that wrapper gives the child pipes rather than the controlling terminal, or proxies stdio, the mouse-enable sequences Textual writes may never reach iTerm2 in a form that turns reporting on, while keyboard input (a separate direction on a separate fd) keeps working. This is the only structural difference between Wilted and a plain Textual app that has not been tested, and it fits the "works under test harnesses, dead in the real terminal" shape exactly.
  - Note the earlier PTY capture does **not** cover this. It compared a trivial Textual app with and without the `env -i` allowlist — it never ran anything through `bws run`.
- Next diagnostic step (needs a real interactive window — not a `!` command inside Claude Code, which is not a TTY that can be clicked into). Run in order; each splits the remaining space in half:
  1. `~/.venvs/wilted/bin/python -m textual` — the demo, same Textual 8.2.3, same venv, launched directly. Mouse dead → a Textual driver problem. Mouse works → Textual and the terminal are both fine, continue to 2.
  2. `~/.venvs/wilted/bin/python -m wilted.cli` from the project root — the real app, **bypassing the `bws run` launcher**. Mouse works → the launcher is the culprit. Mouse dead → Wilted's own app code, with the launcher exonerated.
- Agent constraint worth recording: per the global secret-management rules, `bws run` is an interactive-only path and must not be invoked from agent tool calls, so these two steps cannot be automated — they need the human at the terminal.
