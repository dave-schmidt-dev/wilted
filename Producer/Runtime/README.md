# Wilted — preparation runtime

The Python side of Wilted: feed ingestion, article extraction, transcription,
ad removal, the nightly pipeline, the station runtime and the TUI. The Mac
producer (`../Sources`) and the iOS listener consume what this produces; this
directory is a self-contained project with its own `pyproject.toml`,
virtualenv, `Makefile` and `INVARIANTS.md` (the `INV-N` charter, distinct from
the repository-level `W-INV-NNN` one).

Run its suite from this directory, never from the repository root — the
...
The repository gate runs it as the `test-preparation-runtime` phase0 leg.

Deployer-local settings (NWS location, playback speed, mail) live in a
gitignored `wilted.toml`; this repository is public and carries none of them.
