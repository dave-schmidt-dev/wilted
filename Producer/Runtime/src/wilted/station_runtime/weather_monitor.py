...
Polls the NWS active-alerts feed for the configured forecast zone AND county
(``[weather].zone`` / ``[weather].county`` in ``wilted.toml`` — see
:func:`_resolve_zone_county`; a zone-only query misses county-issued
...
definition and live-verified 2026-07-11: ``?zone=<zone-ugc>,<county-ugc>``
returns a single combined ``FeatureCollection`` covering both UGC codes, vs. repeated
...
from wilted import PROJECT_ROOT, load_config
...
# ---------------------------------------------------------------------------
# Location (deployer configuration) + protocol constants
# ---------------------------------------------------------------------------

_LOCATION_CONFIG_HINT = (
    "Add the NWS UGC codes for your location to wilted.toml:\n"
    "\n"
    "    [weather]\n"
    '    zone = "XXZ000"    # forecast zone UGC\n'
    '    county = "XXC000"  # county UGC\n'
    "\n"
    "Look these up once with: https://api.weather.gov/points/<lat>,<lon> "
    "(properties.forecastZone / county, whose URLs end in the UGC codes)."
)


class WeatherLocationNotConfigured(RuntimeError):
    """The NWS zone/county UGC codes are missing from ``wilted.toml``.

    Raised when a monitor is constructed -- before any poll -- so an
    unconfigured station reports exactly which setting is missing instead of
    silently polling alerts for some other place.
    """


def _resolve_zone_county() -> tuple[str, str]:
    """Read ``zone``/``county`` from ``wilted.toml``'s ``[weather]`` table.

    There is deliberately no default: the deployer's location is their data,
    not the project's, and must not live in a public checkout. Polling the
    wrong area is worse than failing, because it looks like it works.

    Returns:
        ``(zone, county)`` UGC codes.

    Raises:
        WeatherLocationNotConfigured: If either key is absent or empty. The
            message names the missing keys and the file to put them in.
    """
    table = load_config().get("weather", {})
    resolved = {key: str(table.get(key) or "").strip() for key in ("zone", "county")}
    missing = [key for key, value in resolved.items() if not value]
    if missing:
        raise WeatherLocationNotConfigured(
            "NWS alert location is not configured: missing "
            + ", ".join(f"[weather].{key}" for key in missing)
            + f" in {PROJECT_ROOT / 'wilted.toml'}.\n\n"
            + _LOCATION_CONFIG_HINT
        )
    return resolved["zone"], resolved["county"]


def _default_user_agent() -> str:
    """Descriptive User-Agent, per NWS API usage guidance.

    NWS asks callers for a contact address so they can reach the operator of a
...
def make_trigger_file_fetch(trigger_path: Path) -> Callable[[str, str, str], dict]:
    ...
    def fetch(zone: str, county: str, user_agent: str) -> dict:
        ...
        # payload) -- use a natural-reading placeholder so the spoken bulletin
        # reads cleanly during the A.5.1 listening test rather than reciting a
        # UGC code, without naming anyone's actual location in this checkout.
        area_desc = "the test area"
...
def build_production_monitor(*, trigger_path: Path | None = None) -> WeatherMonitor:
    ...
    manual-test hook, not a production branch. Zone/county come from
    ``wilted.toml``'s ``[weather]`` table (:func:`_resolve_zone_county`);
    user_agent/interval stay at their defaults. Does not set ``on_bulletin_ready``
...
class WeatherMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        if zone is None or county is None:
            # Resolve at construction, not at poll time: a poll failure is
            # recorded into last_error/health (a log line), whereas a missing
            # location must fail loudly, before any request is ever made.
            cfg_zone, cfg_county = _resolve_zone_county()
            zone = cfg_zone if zone is None else zone
            county = cfg_county if county is None else county
        self._fetch = fetch
        self._synth = synth
        self._zone = zone
        self._county = county
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--zone",
        default=None,
        help="NWS forecast zone UGC (default: [weather].zone from wilted.toml)",
    )
    parser.add_argument(
        "--county",
        default=None,
        help="NWS county UGC (default: [weather].county from wilted.toml)",
    )
