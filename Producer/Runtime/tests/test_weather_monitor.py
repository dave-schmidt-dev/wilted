...
from wilted.station_runtime.weather_monitor import (
    ...
    _resolve_zone_county,
    build_production_monitor,
    make_trigger_file_fetch,
)

# The deployer's real NWS zone/county live in a gitignored ``wilted.toml``, never
# in the checkout, so the suite carries its own obviously-fictional UGC codes and
...
def _alert_feature(
    *,
    ...
    area_desc="Northwest Example County, ST",
    severity="Severe",
    status="Actual",
    message_type="Alert",
    updated="2026-07-11T12:00:00+00:00",
    headline="Tornado Warning for Northwest Example County",
...
class TestCombinedRequest:
    ...
    def test_alerts_for_zone_and_county_both_admit_from_one_response(self):
        zone_alert = _alert_feature(alert_id="zone-1", area_desc="Northwest Example County, ST")
        county_alert = _alert_feature(alert_id="county-1", area_desc="Example County, ST")
...
class TestWrapperEntrypoint:
    ...
    def test_run_module_invokes_main_and_exits_zero_on_success(self, monkeypatch):
        ...
        with pytest.raises(SystemExit) as exc_info:
            with (
                patch.object(
                    sys,
                    "argv",
                    # Explicit codes keep the wrapper tests hermetic: the real
                    # location lives in a gitignored wilted.toml that a fresh
                    # checkout does not have.
                    ["wilted-weather-monitor", "--zone", TEST_ZONE, "--county", TEST_COUNTY],
                ),
        ...
        assert TEST_ZONE in captured_requests[0].full_url
        assert TEST_COUNTY in captured_requests[0].full_url
...
class TestMakeTriggerFileFetch:
    def test_returns_no_features_when_trigger_file_absent(self, tmp_path):
        ...
        response = fetch(TEST_ZONE, TEST_COUNTY, "wilted-test-agent")
    ...
    def test_returns_one_qualifying_severe_feature_when_trigger_file_present(self, tmp_path):
        ...
        assert props["areaDesc"] == "the test area"  # human-readable, not the UGC code
    ...
    def test_concurrent_fetches_atomically_consume_one_marker(self, tmp_path, monkeypatch):
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            responses = list(
                executor.map(
                    lambda _index: fetch(TEST_ZONE, TEST_COUNTY, "wilted-test-agent"),
                    range(2),
                )
            )
...
class TestLocationResolution:
    """The zone/county come from ``wilted.toml``, never from a shipped default.

    The deployer's location is their data, not the project's, so an
    unconfigured checkout must fail loudly at construction rather than poll
    alerts for somewhere else and look like it is working.
    """

    def test_resolves_zone_and_county_from_the_weather_table(self):
        with patch(
            "wilted.station_runtime.weather_monitor.load_config",
            return_value={"weather": {"zone": TEST_ZONE, "county": TEST_COUNTY}},
        ):
            assert _resolve_zone_county() == (TEST_ZONE, TEST_COUNTY)

    @pytest.mark.parametrize(
        ("table", "expected_keys"),
        [
            ({}, ["[weather].zone", "[weather].county"]),
            ({"zone": TEST_ZONE}, ["[weather].county"]),
            ({"county": TEST_COUNTY}, ["[weather].zone"]),
            ({"zone": "  ", "county": TEST_COUNTY}, ["[weather].zone"]),
        ],
    )
    def test_missing_or_blank_settings_name_themselves(self, table, expected_keys):
        with patch(
            "wilted.station_runtime.weather_monitor.load_config",
            return_value={"weather": table},
        ):
            with pytest.raises(WeatherLocationNotConfigured) as excinfo:
                _resolve_zone_county()

        message = str(excinfo.value)
        for key in expected_keys:
            assert key in message
        assert "wilted.toml" in message

    def test_monitor_construction_fails_before_any_request_is_made(self):
        fetch = _FixtureFetch(_feature_collection())
        ...
        with patch(
            ...
            with pytest.raises(WeatherLocationNotConfigured):
                WeatherMonitor(fetch=fetch, synth=lambda text: BulletinAudio(b"", 0.0))
