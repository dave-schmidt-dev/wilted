...
def _pinned_playback_speed(monkeypatch):
    """Pin the speed indicator so the baselines do not encode operator config.

    ``WiltedApp.__init__`` seeds ``self._speed`` from ``get_default_speed()``,
    which prefers the last-used value in the database, then ``wilted.toml``'s
    ``[playback].speed``. Both are deployer-local, so a machine whose operator
    listens at 1.3x rendered "Speed: 1.3x" into the SVG and failed every
    snapshot -- which is how this coupling was found. Pinning the shipped
